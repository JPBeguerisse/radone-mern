...
import { getUserRequested, getUserFailed, getUserSuccess } from "../reducers/user.reducer";
...
function* getUser(action: PayloadAction<string>) {
    try {
        ...
        const response: AxiosResponse<User> = yield axios.get(`http://localhost:8000/api/user/${action.payload}`,
            {
                headers: {
...
export default function* userSaga() {
    ...
    yield takeLatest(getUserRequested.type, getUser);
}
